...
<!--
Setup
- reset to initial commit (keep git log open on main)
- delete DB (keep ADS open on main)
- open Rider on right, run app without debugging
- open dotTrace on right, leave on main menu
- open Ubuntu on right
- Firefox: slides on right, presenter view on main
-->
...
- focusing almost exclusively on latency
...
- give MVP (24s, 4MB, 172 requests) & Wegmans (20s, 20MB, 459 requests) examples for realism
(dotTrace)
- explain main menu
...
- add async/await everywhere, but don't change anything else
...
- see repeated DB calls at top, cache them
